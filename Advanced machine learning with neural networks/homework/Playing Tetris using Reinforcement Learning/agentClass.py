...
class DQN(nn.Module):
    def __init__(self, state_size, action_size):
        ...
        self.fc1 = nn.Linear(state_size, 64, dtype=torch.float64)
        self.fc2 = nn.Linear(64, 64, dtype=torch.float64)
        self.fc3 = nn.Linear(64, action_size, dtype=torch.float64)
...
class TDQNAgent:
    ...
    def index_to_action(self, action_index):
        ...
        return (int(action_index / 4), action_index % 4)
    ...
    def fn_select_action(self):
        ...
        if np.random.rand() < max(self.epsilon, 1-self.episode/self.epsilon_scale): # self.epsilon: minimum of epsilon
            self.action = random.randint(0, (4*4)-1)
        else: 
            self.action = np.argmax(out)
        
        tile_x, tile_orientation = self.index_to_action(self.action)
    ...
    def fn_reinforce(self,batch):
        ...
        for transition in batch:
            ...
            last_action = transition[1]
            last_reward = transition[2]
            new_state = transition[3]

            new_reward = last_reward
            Q_ground = self.model_target(torch.tensor(new_state)).detach().numpy()
            new_reward += max(Q_ground)
            Q_ground_all.append(torch.tensor(new_reward, dtype=torch.float64))
            Q_pred = self.model(torch.tensor(old_state))
            Q_pred_all.append(Q_pred[last_action])

        Q_ground_all = torch.stack(Q_ground_all)
        Q_pred_all = torch.stack(Q_pred_all)
        
        loss = self.criterion(Q_pred_all, Q_ground_all)
        ...
        if self.gameboard.gameover:
            ...
            if self.episode%1000==0:
                ...
                if self.episode in saveEpisodes:
                    ...
                    torch.save(self.model.state_dict(), 'model_'+str(self.episode)+'.pth')
                    torch.save(self.model_target.state_dict(), 'model_target_'+str(self.episode)+'.pth')
                    with h5py.File("Data_" + str(self.episode) + ".h5", "w") as hf:
                        hf.create_dataset('rewards', data=np.array(self.rewards))
            if self.episode>=self.episode_count:
                raise SystemExit(0)
